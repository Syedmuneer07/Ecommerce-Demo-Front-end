...
function AdminDashboard() {
    ...
    const fetchOrders = async (page = 1) => {
        axios.get(`${import.meta.env.VITE_REACT_APP_BACKEND_API_URL}/api/orders/admin/orders?page=${page}&limit=${limit}`,{
            headers: {
    ...
    const handleStatusChange = async (orderId, newStatus) => {
        try {
            await axios.put(
                `${import.meta.env.VITE_REACT_APP_BACKEND_API_URL}/api/orders/${orderId}`,
                { status: newStatus },
                {
                    headers: {
                        "Authorization": `Bearer ${localStorage.getItem("token")}`
